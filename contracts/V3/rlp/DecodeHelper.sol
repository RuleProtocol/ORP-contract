...
/*
* Used to proxy function calls to the RLPDecode for testing
*/
...
contract DecodeHelper {
    ...
    function isList(bytes memory item) public pure returns (bool) {
        RLPDecode.RLPItem memory rlpItem = item.toRlpItem();
    ...
    function numItems(bytes memory item) public pure returns (uint) {
        RLPDecode.RLPItem[] memory rlpItem = item.toRlpItem().toList();
    ...
    function nestedIteration(bytes memory item) public pure returns (string memory) {
        RLPDecode.Iterator memory iter = item.toRlpItem().iterator();
        RLPDecode.Iterator memory subIter = iter.next().iterator();
    ...
    function toBlockHeader(bytes memory rlpHeader) public pure returns (
        ...
        RLPDecode.Iterator memory it = rlpHeader.toRlpItem().iterator();
        uint idx;
        while(it.hasNext()) {
